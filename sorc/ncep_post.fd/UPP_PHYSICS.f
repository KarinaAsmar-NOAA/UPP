...
!        deallocate (wrk1, wrk11, wrk2, wrk3, cosl, iw, ie)
      ...
      ELSE ! IF(MODELNAME == 'GFS' .or. global)
...
!  is there a (f+tan(phi)/erad)*u term?
      ...
      DO I=ISTA,IEND
         ...
         rtmp=min(rtmp,abs(PSI(I,J)))
         rerr=max(rerr,abs(PSI(I,J)-PTMP(I,J)))
         ENDIF  
      ENDDO
      ENDDO
      ENDDO
      ...
      DO I=ISTA,IEND
         IF(J>1 .and. J<JM) THEN
         CHI(I,J) = 0.25*( CHI(I-1,J)+CHI(I+1,J)+CHI(I,J-1)+CHI(I,J+1)  &
                  - DIV(I,J)/(wrk2(i,j)*wrk1(i,j)*wrk3(i,j)*wrk1(i,j)*COSL(i,j)*4.))
         rerr=max(rerr,abs(CHI(I,J)-PTMP(I,J)))
         ENDIF  
      ENDDO
      ENDDO
      ENDDO
